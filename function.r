...
Metadata_Pipeline <- function(df, my_dataset, endpoint){
 ...
 #df <- total_rescription #THIS LINE IS FOR DEUBGGING
...
SELECT * WHERE {
   ...
   # blank_nodes_content <-   blank_nodes_content %>% mutate(Publisher = NA, Datatype=NA, ReqLevel =NA, Explanation = NA, num_min_max_entries = NA)
    blank_nodes_content
  # df_combined <-  df_combined %>% mutate(Property = URI) %>% select(-URI)
...
df_conformity <- left_join(df_reference,df_combined, by = c("Property"= "Property")) 
  
df_combined <- df_conformity %>% add_row(df_combined[!(df_combined$Property %in% df_conformity$Property),])
  
 df_combined #%>% select(-publisher.y) 
